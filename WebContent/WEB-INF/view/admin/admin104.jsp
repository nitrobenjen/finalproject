	...
	$(document).ready(function() {
		...
		//강의실 삭제
		 $(".btndelete").on("click", function() {
			...
			var class_id = $(this).parents("tr").find("td:eq(0)").text();
			...
			$(".delForm #class_id").val(class_id);
			$("#ca-del-Modal").find("input#class_id").val(class_id); 
			 $("#ca-del-Modal").find("h5#class_id").text(class_id);
			$("#ca-del-Modal").find("h4#class_name").text(class_name);
		...
		//강의실 수정
		 $(".btnupdate").on("click", function() {
			...
			$(".upForm #class_id2").val(class_id);
			
			$(".upForm #focusedInput").attr("placeholder", class_name);
			$(".upForm #focusedInput2").attr("placeholder", jungwon);
			
			
			$("#ca-mod-Modal").find("input#class_id").val(class_id); 
			 $("#ca-mod-Modal").find("h5#class_id").text(class_id);
			$("#ca-mod-Modal").find("h4#class_name").text(class_name);
	...
	<div id="main">
		...
		<div class="content">
			...
			<table class="table table-striped table-bordered ocu">
				...
				<tbody>
					<tr>
						<c:forEach var="in" items="${infoclass}">
							<tr>
								<td>${in.class_id }</td>
								<td>${in.class_name }</td>
								<td>${in.jungwon }</td>
								<td><button type="button" class="btn btn-default btnupdate"
										data-toggle="modal" data-target="#ca-mod-Modal" value="${in.class_id}">수정</button></td>
								<td><button type="button" class="btn btn-default btndelete"
										data-toggle="modal" data-target="#ca-del-Modal" value="${in.class_id}">삭제</button></td>
							</tr>
						</c:forEach>
				</tbody>
			</table>
	...
	<div class="modal fade" id="ca-insert-Modal" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				...
				<form action="${pageContext.request.contextPath}/adminclassinsert.do" method="post" class="form-horizontal">
					<div class="modal-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input class="form-control" name ="class_name"id="focusedInput" type="text">
							</div>
							<label class="control-label col-sm-2" style="margin-top: 10px;">정원</label>
							<div class="col-sm-10" style="margin-top: 10px;">
								<input class="form-control" name= "jungwon" id="focusedInput" type="text">
							</div>
						</div>
	...
	<div class="modal fade" id="ca-mod-Modal" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h5 class="modal-title" id="class_id"></h5>강의실 수정
				</div>
				<form action="${pageContext.request.contextPath}/adminclassupdate.do"
					method="post" class="form-horizontal upForm">
					<div class="modal-body">

						<h4 id="class_name" style="text-align: center; font-weight: bold;"></h4>
						...
						<div class="form-group">
							<label class="control-label col-sm-2">강의실명</label>
							<div class="col-sm-10">
								<input class="form-control" id="focusedInput" type="text" name="class_name">
							</div>
							<label class="control-label col-sm-2" style="margin-top: 10px;">정원</label>
							<div class="col-sm-10" style="margin-top: 10px;">
								<input class="form-control" id="focusedInput2" type="text" name="jungwon">
							</div>
						</div>

					</div>
					<div class="modal-footer">
					<input type="hidden" id="class_id2" name="class_id">	
	...
	<div class="modal fade" id="ca-del-Modal" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h5 class="modal-title" id="class_id" ></h5>강의실 삭제
				</div>
				<div class="modal-body">
				...
				<div class="modal-footer">
				<form
					action="${pageContext.request.contextPath}/adminclassdelete.do"
