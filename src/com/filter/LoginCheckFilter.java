...
public class LoginCheckFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		command = command.substring(1, command.indexOf(".do"));
		...
		if (session != null) {
			if (session.getAttribute("logininfo") != null) {
				// 관리자 확인
				if (command.startsWith("admin") && ((LoginInfo) session.getAttribute("logininfo")).getGrade() == 0) {
					// 로그인 사용자 인증을 위한 값 저장
					login = true;
					// 강사 확인
				} else if (command.startsWith("teach")
						&& ((LoginInfo) session.getAttribute("logininfo")).getGrade() == 1) {
					login = true;
					// 학생확인
				} else if (command.startsWith("student")
						&& ((LoginInfo) session.getAttribute("logininfo")).getGrade() == 2) {
					login = true;
				}
			}
		}
		...
		if (login) {
			...
			RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/view/loginfail.jsp");
